Here's the current README.md content:
...
```json
{
  "answer": "Refund and Return Policy Our refund policy allows customers to return most products within 30 days...",
  "sources": [
    {
...
Queries the index with a question and returns a concise answer extracted from the retrieved documents, along with source information including document name, chunk ID, relevance score, and a confidence field.
...
Response fields:
- `answer` — The extracted answer from the documents
- `sources` — List of source chunks with document name, chunk ID, and similarity score
- `confidence` — Either `"answered_from_docs"` or `"insufficient_context"`
...
The file is at `e:\学习\code\rag-service\README.md` — you can open it in VS Code to view or edit.# 22
